...
<body>
	...
	//koneksi ke database
	$conn = new mysqli("localhost", "root", "", "ta10");
	...
	if($_POST['submit']){
		...
		$cek 			= $conn->query("SELECT password FROM webprog WHERE password='$password_lama'");
		...
		if($cek->num_rows){
			...
			if(strlen($password_baru) >= 5){
				...
				if($password_baru == $konfirmasi_password){
					...
					$update 		= $conn->query("UPDATE webprog SET password='$password_baru' WHERE nim='$nim'");
					if($update){
